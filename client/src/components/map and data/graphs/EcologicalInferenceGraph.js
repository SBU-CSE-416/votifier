...
export default function EcologicalInferenceGraph() {
    ...
    const fetch_ei_race = async (stateAbbreviation, racialGroup, regionType) => {
        try {
            const response = await fetch(`https://votifier-server-production.up.railway.app/api/data/${stateAbbreviation}/ei-analysis/demographics/${racialGroup}/${regionType}`);
    ...
    const fetch_ei_economic = async (stateAbbreviation, economicGroup, regionType) => {
        try{
            const response = await fetch(`https://votifier-server-production.up.railway.app/api/data/${stateAbbreviation}/ei-analysis/economics/${economicGroup}/${regionType}`);
            const json = await response.json();
